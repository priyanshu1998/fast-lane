...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        std::cout << "follower: attached to '" << channel.name() << "' with " << channel.capacity()
                  << " bytes of capacity\n";
        ...
        while (max_updates == 0 || updates < max_updates)
        {
            ...
            std::cout << "follower: shared sequence " << snapshot.sequence << ", copied "
                      << snapshot.bytes.size() << " bytes, values ";
            print_values(values);
